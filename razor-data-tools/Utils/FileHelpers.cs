...
namespace razor_data_tools.Utils
{
    public static class FileHelpers
    {
        ...
        public static async Task<byte[]> ProcessFormFile<T>(IFormFile formFile, ModelStateDictionary modelState)
        {
            ...
            string importFileName = formFile.FileName;
            ...
            try
            {
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    else
                    {
                        ...
                        Utils.Csv.ImportBMIEarningsCsvAndInsertIntoDatabase(memoryStream, importFileName);
        ...
        public static async Task<byte[]> ProcessStreamedFile(MultipartSection section, ContentDispositionHeaderValue contentDisposition, ModelStateDictionary modelState, string[] permittedExtensions, long sizeLimit)
        {
            try
            {
                using (var memoryStream = new MemoryStream())
        ...
        //{
        //    //If implementing this method be sure to switch this variable's value with one from the config file.
        //    string _allowedChars = string.Empty;
        ...
        //        return false;  // signatures.Any(signature =>  headerBytes.Take(signature.Length).SequenceEqual(signature));
        //    }
        //}
    }
}
